...
df_denom_1 = df_denom.groupby([pd.to_datetime(df_denom['Departure_Date']).dt.strftime('%Y-%m'), 'Der_Provider_Code'])['EC_Departures'].sum().reset_index()
...
df_join_1['Emergency care FHIR ToC utilisation (per 1,000 discharges)'] = df_join_1["Number of successful FHIR ToC emergency care discharge messages"]/ (df_join_1['Number of emergency care patient discharges']/1000)
