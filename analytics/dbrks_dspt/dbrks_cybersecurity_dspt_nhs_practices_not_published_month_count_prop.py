...
df = df[df["Organisation Latest DSPT Status"] == "Not Published"].reset_index(drop=True)
...
df_output = pd.DataFrame(Data_f,
    columns=[
        "DSPT status",
        "No of GP practices that have not submitted a 20/21 DSPT assessment",
        "Total number of Practices",
        "Percent of GP practices that have not submitted a 20/21 DSPT assessment",
        "Date",
    ],
)
